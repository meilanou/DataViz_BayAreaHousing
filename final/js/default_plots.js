...
/* basic x-axis with no tick */
function setup_simple_xaxis(g, width, height, margin, extent) {
...
/* basic y-axis with no tick */
function setup_simple_yaxis(g, height, margin, extent) {
...
function amend_series_line_chart(g, line_func, obj, x_scale, y_scale, color, format_pattern) {
  ...
  var focus_g = g.append("g")
      .attr("class", "focus")
      .style("display", "none");

  focus_g.append("circle")
      .attr("r", 4.5);

  var bg_def = focus_g.append('defs')
    .append('filter')
    .attr('x', 0)
    .attr('y', 0)
    .attr('width', 1)
    .attr('height', 1)
    .attr('id', 'text_bg');
  bg_def.append('feFlood')
    .attr('flood-color', 'white');
  bg_def.append('feComposite')
    .attr('in', 'SourceGraphic');

  focus_g.append("text")
    .attr('y', '-30px')
    .attr("dy", ".35em")
    .attr('filter', 'url(#text_bg)');
  ...
  function mousemove() {
    var format = get_nice_format(format_pattern);
    // find left of bisect index of binded data array for a given x value
    var bisect_date = d3.bisector(function(d) { return d['period']; }).left;

    var x_date = x_scale.invert(d3.mouse(this)[0]), // find x date value by mouse location
      i = bisect_date(data, x_date); // find left of bisect index for given x date value
    var d0 = data[(i > 0 ? i - 1 : i)],
      d1 = data[i],
      d = x_date - d0['period'] > d1['period'] - x_date ? d1 : d0, // find closer data point
      x_date = d['period'], // djust x_date with a real date point
      y_val = d['datum']; 
    
    // move tooltip to data point location
    focus_g.attr("transform", 
      "translate(" + x_scale(x_date) + "," + y_scale(y_val) + ")");
    // display tooltip text
    var txt = focus_g.select("text");
    // avoid hidden text
    if (i >= data.length/2) { txt.attr('text-anchor', 'end'); }
    else if (i < data.length/2) { txt.attr('text-anchor', 'start'); }
    // text content
    // first date of next month is used to represent current month period
    txt.text(Month_Names_Abbr[x_date.getMonth()-1] + ' ' + 
      x_date.getFullYear() + ': ' + format(y_val));
  }
}
...
/* draw buttons (text + rect) on svg with click event handlers */
function draw_buttons(svg, x, y, width, array) {
  var height = 26;
  var g = svg.append('g')
    .attr('class', 'svg_button');
